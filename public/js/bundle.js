...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    container: 'map',
    // Here this container will be set to element with ID map
    style: 'mapbox://styles/subham-jaiswal/ckvmuhmhw3gks14pirhkrwtlo',
    scrollZoom: false
  }); // Map Creation

  var bounds = new mapboxgl.LngLatBounds(); // SYNTACTICAL

  locations.forEach(function (loc) {
    var el = document.createElement('div'); // Craeting those green arrow points used marker class
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p> Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // Creating Pop-ups 
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:1000/api/v1/users/UpdatePassword' : 'http://127.0.0.1:1000/api/v1/users/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
var stripe = Stripe('pk_test_WbTsVdoveoNnO4deE85I7nyG');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("http://127.0.0.1:1000/api/v1/bookings/checkout-session/".concat(tourId));
...
var updatePass = document.querySelector('.form.form-user-password');
...
if (updateForm) {
  ...
  updateForm.addEventListener('submit', function (e) {
    ...
    (0, _updateSettings.update)(data, 'data');
  });
}
...
if (updatePass) {
  ...
  updatePass.addEventListener('submit', function (e) {
    ...
    (0, _updateSettings.update)(data, 'password');
  });
}
